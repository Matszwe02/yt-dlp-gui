...
"""
import logging, threading, os, sys, shutil, time
...
class MainWindow(qtw.QMainWindow, Ui_MainWindow):
    ...
    def save_preset(self):
        ...
        if "autostart" in self.preset:
            self.config["autostart"] = self.cb_auto.isChecked()
        if "ctrlv" in self.preset:
            self.config["ctrlv"] = self.cb_ctrlv.isChecked()
        if "embedsubs" in self.preset:
            self.config["embedsubs"] = self.cb_subtitlesembed.isChecked()
        if "mkvremux" in self.preset:
            self.config["mkvremux"] = self.cb_mkvremux.isChecked()
        if "clipboardmonitor" in self.preset:
    ...
    def load_preset(self, fmt):
        ...
        if "mkvremux" in preset:
            self.cb_mkvremux.setEnabled(True)
            self.cb_mkvremux.setChecked(preset["mkvremux"])
        else:
        ...
        if "ctrlv" in preset:
            ...
            self.cb_ctrlv.setChecked(preset["ctrlv"])
        else:
            self.cb_ctrlv.setChecked(False)
            self.cb_ctrlv.setEnabled(False)

        if "autostart" in preset:
            self.cb_auto.setEnabled(True)
            self.cb_auto.setChecked(preset["autostart"])
        else:
        ...
        if "clipboardmonitor" in preset:
            self.cb_clipboardmonitor.setEnabled(True)
            self.cb_clipboardmonitor.setChecked(preset["clipboardmonitor"])
        else:
